...
class Code
{
    ...
    public function getCode()
    {
        ...
        return '
		Ext.ns("' . $this->project->namespace . '","' . $this->project->runnamespace . '");
		' . $code['defines'] . '
	    ' . $code['layout'].'
	    ' . $this->project->getActionJs();
    }
        ...
        if($store instanceof \Ext_Helper_Store){
            ...
            switch ($store->getType()){
                case \Ext_Helper_Store::TYPE_INSTANCE :
                    $object->store = 'Ext.create("'.\Ext_Code::appendNamespace(trim(str_replace(self::$NEW_INSTANCE_TOKEN, '', $store->getValue()))).'",{})';
                    break;
            ...
            $store = trim($store);
            if(strpos($store , self::$NEW_INSTANCE_TOKEN) !==false){
                $object->store = 'Ext.create("'.\Ext_Code::appendNamespace(trim(str_replace(self::$NEW_INSTANCE_TOKEN, '', $store))).'",{})';
            }elseif (strlen($store)){
    ...
    protected function _compileJs($parent)
    {
        ...
        if($parent === 0)
        {
            if($this->project->itemExist(Project::COMPONENT_ROOT) && $this->project->hasChildren(Project::COMPONENT_ROOT))
            {
                foreach($this->project->getChildren(Project::COMPONENT_ROOT) as $itemData)
                {
                    $item = $itemData['data'];
                    $item->extendedComponent(true);
                    $result = $this->_compileExtendedItem($itemData['id'], Project::COMPONENT_ROOT);
                    $definesCode.= $result['defines'];
                }
            }
            $parent = Project::LAYOUT_ROOT;
        }

        if($this->project->hasChildren($parent))
        {
            $childs = $this->project->getChildren($parent);

            if($parent == Project::LAYOUT_ROOT && !empty($childs)){
            ...
            foreach($childs as $item)
            {
                ...
                switch($oClass)
                {
                    case 'Docked' :
                        ...
                        if(!$this->project->hasChildren($item['id']))
                            break;
                         ...
                         */
                        $docked = $this->project->runnamespace . '.' . $itemObject->getName();
                        break;
                    ...
                    default:
                        $result = $this->_compileItem($item['id']);
                        $layoutCode .= $result['layout'];
                        $items[] = $this->project->runnamespace . '.' . $itemObject->getName();
        ...
        if($parent !== 0)
        {
            $parentObject = $this->project->getItemData($parent);
    ...
    public function getObjectDefineJs($id)
    {
        ...
        $object = $this->project->getObject($id);
        ...
        {
            $manager = $this->project->getMethodManager();
            ...
            $manager = $this->project->getEventManager();
        ...
        return $this->project->getObject($id)->getDefineJs($this->project->namespace);
    }
    ...
    protected function _compileConfig($name)
    {
        ...
        if($this->project->hasChildren($name))
        {
            $children = $this->project->getChildren($name);
    ...
    {
        $object = $this->project->getItemData($id);
        $hasChilds = $this->project->hasChildren($parent);
    ...
    protected function _compileExtendedSubItems($parent , $mainContainer)
    {
        ...
        $mainContainerObject = $this->project->getItemData($mainContainer);
        ...
        if($parent!=='0')
        {
            $container = $this->project->getItemData($parent);
    ...
    {
        if(!$this->project->objectExists($name))
            return '';

        $this->applyStoreInstances();

        $object = $this->project->getObject($name);
        $oClass = $object->getClass();

        if(in_array($oClass , Project::$defines , true) || $object->isExtendedComponent())
