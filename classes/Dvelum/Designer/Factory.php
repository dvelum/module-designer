...
class Factory
{
     ...
     * @throws \Exception
     */
    static public function loadProject(ConfigInterface $designerConfig , $projectFile) : ?Project
    {
        $storage = Storage::factory($designerConfig->get('storage') , $designerConfig);
     ...
     * @throws \Exception
     */
    static public function getStorage(ConfigInterface $designerConfig): Storage
    {
        return Storage::factory($designerConfig->get('storage') , $designerConfig);
    }
    ...
    static public function runProject($projectFile , ConfigInterface $designerConfig , $replace = array(), $renderTo = false, $moduleId = false)
    {
        ...
        $project = Factory::loadProject($designerConfig, $projectFile);
    ...
    static public function compileDesktopProject($projectFile , ConfigInterface $designerConfig , $replace, $renderTo, $moduleId)
    {
        ...
        if(!empty($includes))
        {
            $wwwRoot = Request::factory()->wwwRoot();
     ...
     * @return array
     */
    static public function getProjectIncludes($cacheKey , Project $project , $selfInclude = true , $replace = array() , $debug = false)
    {
        $applicationConfig = Config::storage()->get('main.php');
        $designerConfig = Config::storage()->get('designer.php');
        $manager = new Manager($applicationConfig);
        ...
        if(isset($projectConfig['files']) && !empty($projectConfig['files']))
        {
            foreach ($projectConfig['files'] as $file)
            {
                ...
                }else
                {
                    ...
                    $subProject = Factory::loadProject($designerConfig,  $projectFile);
